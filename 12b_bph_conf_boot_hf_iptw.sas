/************************************************************************************************
	...
	PROGRAM: 12b_bph_conf_boot_hf_iptw.sas
	DESCRIPTION: Bootstrap the RR and RD estimates for HF (only with
	IPTW) while conducting asymmetric propensity score trimming.

	We didn't think it informative to run the assymetric trimming analyses without weighting.
...
%setup(full, 10b_ps_trim_boot_hf, saveLog=Y);
...
%competingrisk_weights(inds=ana.primary_cohort, startDT=FillDate2, eventDT=HF_date, crDT=death_dt, censorDT=endDT_itt, 
      daysEst=365, psvars=age cal_year bl_aur bl_tobacco bl_chdz /*bl_hf*/ bl_hf_hosp bl_chronickid bl_copd bl_hchl 
				...
				bl_hospBed bl_outpatientvisit bl_rehab bl_oxygen bl_wheelchair cat_bph_days num_bph, 
				...
				bl_ambulance bl_hospBed bl_outpatientvisit bl_rehab bl_oxygen bl_wheelchair cat_bph_days num_bph, trtvar=ab, 
				noiptw=0,
      numiterations=500, outds=ana.hf_365_bph_c_boot);
