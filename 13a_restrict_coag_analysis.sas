/************************************************************************************************
	...
	PROGRAM: 13a_restrict_coag_analysis.sas
	DESCRIPTION: The goal of this analysis is to replicate the primary analyses, excluding 
	those new-use episodes with a history of anticoagulant use in the last year.

	This code is used to derive the cohort and print out their table 1.
...
data ana.p_cohort_restrict_coag;
set ana.primary_cohort;
	
	if bl_anycoag = 1 then delete;

run;

*Dropped from 189,886 persons to 163,320 persons/new-use episodes;
...
proc sort data=ana.p_cohort_restrict_coag out=p_cohort_restrict_coag; by bene_id indexdate; run;
%table1(inds = p_cohort_restrict_coag, maxLevels = 5, colVar = ab, rowVars = age cal_year rti_race
...
ods rtf file="&OutPath./Table 1: Distribution of Study Baseline Characteristics_No Anticoagulants_&logdate..rtf" style=vpmin startpage=never ;
