...
%macro risk_weights_multiple_sample(inds=cohort, startDT=indexDate, censorDT=censorDT, 
      daysEst=365, psvars=age sex race, dovars=age sex race, classvars=sex race, trtvar=trt, 
      numiterations=1, cohort=_sample, noiptw=0);
...
/*   %END;*/
   ...
   %DO i=1 %TO &numIterations;
	  ...
	  *STEP 3a: CALCULATE FIRST PS MODEL FOR TRIMMING - PS model is agnostic of outcome so same for all;
      ...
      *STEP 4: CREATE DROP OUT WEIGHTS - Do one model for each outcome;
