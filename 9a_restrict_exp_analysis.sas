/************************************************************************************************
	...
	PROGRAM: 9a_restr_exp_analysis.sas
	DESCRIPTION: The goal of this analysis is to replicate the primary analyses, excluding 
	those new-use episodes that are attributed to meds that aren't selective alpha-1a blockers.
...
*Remove the episodes that we're not interested in;
data ana.p_cohort_restrict_exp;
set ana.primary_cohort;
	
	if ab = 1 & gnn in ("ALFUZOSIN","DOXAZOSIN","Multiple","PRAZOSIN","TERAZOSIN") then delete;

run;

*Dropped from 189,886 persons to 167,438 persons/new-use episodes;
...
%ps(inds = ana.p_cohort_restrict_exp, outds = ps);
...
ods graphics on / reset imagename="PS Distributions - Density - Restricted Exp";
proc sgplot data=ps;
...
*Look at a plot of the PSs;
ods graphics on / reset imagename="PS Distributions - Density - Restricted Exposure - Trimmed";
proc sgplot data=ps_trim;
	title "Density - PS Distributions by Treatment Group in the Restricted Exposure Cohort After Trimming";
	*histogram ps / group = ab;
...
ods rtf file="&OutPath./Table 1: Distribution of Study Baseline Characteristics_Trimmed_Restricted Exp Population_&logdate..rtf" style=vpmin startpage=never ;
...
ods noptitle; ods escapechar='~';Title ;
ods rtf file="&OutPath./Table 1: Distribution of Study Baseline Characteristics with IPTW_Trimmed_Restricted Exp Population_&logdate..rtf" style=vpmin startpage=never ;
