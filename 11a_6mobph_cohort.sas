/************************************************************************************************
	...
	PROGRAM: 11a_6mobph_cohort.sas
	DESCRIPTION:The goal of this program is to derive the cohort for a sensitivity analysis where
	we require a BPH diagnosis in the 6 months (180 days) prior to new-use of the study 
	medication.

	Ultimately, we decided that the cohort was not different enough from the primary cohort to
	justify re-running the primary analyses. We had prespecified that we would make this decision
	ad hoc.
...
proc sql;
	...
	select *
	from cohort_restrict
	where exclude_6moslb_BPHWOUS ge 1 or exclude_6moslb_BPHWUS ge 1
	;
	quit;

***THIS IS DIFFERENT FROM THE PRIMARY COHORT FILE.
...
Print out the Table 1 for this patient population.

***************************************************************/


proc sort data=bph6mo_cohort out=bph6mo_cohort_sort; by bene_id indexdate; run;
%table1(inds = bph6mo_cohort_sort, maxLevels = 5, colVar = ab, rowVars = age cal_year rti_race
...
ods rtf file="&OutPath./Table 1: Distribution of Study Baseline Characteristics_6 Mo LB BPH_&logdate..rtf" style=vpmin startpage=never ;
...
Look at prescription fills over time to determine if it looks
meaningfully different from the primary cohort.

***************************************************************/



****Look at the distribution of new-use medications over time;
