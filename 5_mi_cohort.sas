/************************************************************************************************
	...
	PROGRAM: 5_mi_cohort
	DESCRIPTION: Derive the study cohort that experienced a MI in the last 12 months.
...
%setup(full, 5_mi_cohort, saveLog=N);
...
proc sql;
	...
	quit;
	*189,886 people - UPDATED; 
	*197,684 episodes - UPDATED;




*Limit the dataset to new-use episodes with a history of myocardial infarction;
proc sql;
	create table cohort_restrict3_mi as
	select *
	from cohort_restrict3
	where bl_1yrlb_mi_inpt = 1;
	quit;



*Count the unique number of people and episodes in cohort_restrict3_mi;
proc sql;
	select count (distinct bene_id) as num_people,
		   count (bene_id) as num_episodes
	from cohort_restrict3_mi;
	quit;
	*2,807 people - UPDATED; 
	*2,814 episodes - UPDATED;
...
*Merge the outcomes dataset onto the new MI cohort;

proc sql;
	create table ana.mi_cohort as
	select a.*, b.*
	from cohort_mi as a
	left join out.outcomes as b
