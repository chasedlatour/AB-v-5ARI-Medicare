...
%macro competingrisk_single_weights(inds=cohort, startDT=indexDate, eventDT=outcomeDT, crDT=deathDT, censorDT=censorDT, 
   ...
   *1b: Jitter tied event times;
   ...
   %DO i=1 %TO &numIterations;
      *STEP 2: CREATE BOOTSTRAP SAMPLES
   		Remove thsi because want for the primary sample;
   	  data _anacohort&i; set _anacohort; _id=_N_; run;
      ...
      *STEP 4: CREATE DROP OUT WEIGHTS
      ...
      run;
      proc datasets lib=work nolist nodetails; delete ___anacohort&i; run; quit;
...
/*      %ELSE %DO; proc append base=&outds data=_rd&i; run; %END;*/
      ...
      proc datasets lib=work nolist nodetails;  run; quit;
