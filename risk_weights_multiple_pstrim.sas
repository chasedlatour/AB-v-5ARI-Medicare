...
%macro risk_weights_multiple_pstrim(inds=cohort, startDT=indexDate, censorDT=censorDT, 
...
*STEP 0: Parse macro variables;
   ...
   %DO d=1 %TO &numEst; %LET est&d = %SCAN(&daysEst,&d); %END;
	...
	%PUT Prepare analytic cohort;

*STEP 1: PREPARE ANALYTIC COHORT DATASET;
...
/*         %LET i = %EVAL(&i+1);*/
/*         data _anacohort; set _anacohort; call streaminit(123+&i); days=days+rand("uniform")*.0055-.00275; run;*/
/*      %END;*/
/*   %END;*/

   %PUT Start bootstrapping, full print only for first iteration;

   %DO i=1 %TO &numIterations;

   		%IF &i>1 %then options nomlogic nomprint nosymbolgen nonotes;;

      *STEP 2: CREATE BOOTSTRAP SAMPLES - This will be used for all outcomes;
	  ...
	  *STEP 3b: TRIM POPULATION WITH STURMER TRIMMING;
	  %put STEP 3b: TRIM POPULATION WITH STURMER TRIMMING;
	  *Calculate p5 and p95 ps by treatment arm;
	  proc sort data=_ps; by &trtvar; run;
	  proc means data=_ps noprint;
	  	by &trtvar;
		var ps;
		output out=_sumstat p5=PS5 p95=PS95;
	  run;

	  *Create dataset with the treated 5th percentile;
	  data _p05;
	  set _sumstat (where = (ab=1));
	  	keep ps5;
	  run;
	  *Create dataset with the untreated 95th percentile;
	  data _p95;
	  set _sumstat (where = (ab=0));
	  	keep ps95;
	  run;
	  ...
	  *Merge the datasets so that can remove people;
	  ...
	  merge _ps _p05 _p95;
	  run;
		*Only merge onto the first row;

	  *Drag the values down the table so that the PS values are retained for every
	  	person in the dataset;
	  data _ps2_2 (drop = filled5 filled95);
	  set _ps2;
	  	retain filled5 filled95;
		if ps5 ne . then filled5 = ps5;
		if ps95 ne . then filled95 = ps95;
		ps5 = filled5;
		ps95=filled95;
	  run;
	  ...
	  set _ps2_2;
	  	delete=0;
		if ps < ps5 then delete=1;
		if ps > ps95 then delete=1;
	  run;
